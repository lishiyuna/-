<template>
  <el-form :model="state.form" ref="resetFormData">
    <el-form-item label="分类名称" prop="title">
      <el-input placeholder="请输入分类名称" v-model="state.form.title" />
    </el-form-item>
...
//引入封装删除获取菜单api
import { addBanner, getBanner, editBanner } from "@/api/banner.js";
...
onMounted(async () => {
  ...
  if (props.editID) {
    ...
    let res = await getBanner(props.editID);
    state.form = res;
    // 回显图片
    let url = state.form.img ? proxy.$HOST + state.form.img : "";
    if (url) {
      state.fileList.push({ name: state.form.title, url });
    }
  }
});
...
const onSubmit = async (data) => {
  ...
  if (props.editID) {
    ...
    try {
      res = await editBanner(formdata); //编辑请求
    } catch (error) {
      console.log(error);
    }
  } else {
